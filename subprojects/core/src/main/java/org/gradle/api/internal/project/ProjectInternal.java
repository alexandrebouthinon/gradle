...
public interface ProjectInternal extends Project, ProjectIdentifier, FileOperations, ProcessOperations, DomainObjectContext, DependencyMetaDataProvider, ModelRegistryScope, PluginAwareInternal {
     ...
     * {@link ProjectEvaluationListener} batch which is returned at the end if not empty.
     *
     * @param batch the current listener batch
     * @param action the batch evaluation action
     ...
     */
    @Nullable
    ProjectEvaluationListener stepEvaluation(ProjectEvaluationListener batch, Action<ProjectEvaluationListener> action);
}
